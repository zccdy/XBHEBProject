...
- (void)setRefreshState:(XHRefreshState)refreshState {
    switch (refreshState) {
        ...
        case XHRefreshStateNormal: {
            switch (self.pullDownRefreshViewType) {
                case XHPullDownRefreshViewTypeCircle:
                    self.refreshCircleContainerView.stateLabel.text = self.refreshStringNormal;
                    break;
                case XHPullDownRefreshViewTypeActivityIndicator:
                    break;
                case XHPullDownRefreshViewTypeCustom:
        ...
        case XHRefreshStateLoading: {
            if (self.pullDownRefreshing) {
                if (self.pullDownRefreshViewType == XHPullDownRefreshViewTypeCircle) {
                    self.refreshCircleContainerView.stateLabel.text = self.refreshStringLoading;
                }
        ...
        case XHRefreshStatePulling:
            switch (self.pullDownRefreshViewType) {
                case XHPullDownRefreshViewTypeCircle:
                    self.refreshCircleContainerView.stateLabel.text = self.refreshStringPulling;
                    break;
                case XHPullDownRefreshViewTypeActivityIndicator:
                    break;
                case XHPullDownRefreshViewTypeCustom:
...
- (id)initWithScrollView:(UIScrollView *)scrollView delegate:(id <XHRefreshControlDelegate>)delegate {
    ...
    if (self) {
        ...
        self.refreshStringNormal=@"下拉刷新";
        self.refreshStringLoading=@"正在加载";
        self.refreshStringPulling=@"释放立即更新";
        [self setup];
    }
    return self;
}
...
- (void)dealloc {
    self.refreshStringNormal=nil;
    self.refreshStringPulling=nil;
    self.refreshStringLoading=nil;
    self.delegate = nil;
