...
- (void)makeToast:(NSString *)message DownMoveToCenterDuration:(CGFloat)moveTime NotifyAppearDelay:(CGFloat)appearDelay HideDelay:(CGFloat)delay{
    ...
    [self showToast:toast DownMoveToCenterDuration:moveTime NotifyAppearDelay:appearDelay HideDelay:delay];
}
...
- (void)showToast:(UIView *)toast DownMoveToCenterDuration:(CGFloat)moveTime NotifyAppearDelay:(CGFloat)appearDelay HideDelay:(CGFloat)delayTime{
    toast.center = [self centerPointForPosition:@"bottom" withToast:toast];
    toast.alpha = 0.0;
/*
    if (appearDelay>0.0) {
    ...
    [UIView animateWithDuration:moveTime
                     ...
                     animations:^{
                         toast.alpha = 1.0;
                         toast.center = [self centerPointForPosition:@"center" withToast:toast];
                     } completion:^(BOOL finished) {
                         [UIView animateWithDuration:CSToastFadeDuration
                                               delay:delayTime
                                             options:UIViewAnimationOptionCurveEaseIn
...
/*
-(void)AppearAnimate:(UIView *)toast{
    [UIView beginAnimations:@"toast Appear" context:nil];
    [UIView setAnimationDelegate:self];
    [UIView setAnimationDidStopSelector:@selector(AppearAnimateEnd)];
}

-(void)AppearAnimateEnd{
